...
class EventMonitor(HandleNotification):
    ...
    async def handle(self, relay_url: str, subscription_id: str, ev: Event):
        ...
        elif event_kind == 6109 or (event_kind >= 6000 and event_kind < 7000):  # Result event
    ...
    async def _process_request_event(self, relay_url: str, event: Event):
        ...
        print(f"REQUEST: {event_id[:8]}... - {description[:50]}{'...' if len(description) > 50 else ''}")
    ...
    async def _process_offer_event(self, relay_url: str, event: Event):
        ...
        if referenced_request:
            ...
            print(f"OFFER for {referenced_request[:8]}... - Price: {price} sats - {event.content()[:30]}{'...' if len(event.content()) > 30 else ''}")
    ...
    async def _process_result_event(self, relay_url: str, event: Event):
        ...
        if referenced_request:
            request_desc = self.request_events[referenced_request]["description"]
            print(f"RESULT for {referenced_request[:8]}... - Kind: {event.kind().as_u16()} - {event.content()[:30]}{'...' if len(event.content()) > 30 else ''}")
...
async def main():
    ...
    event_filter = Filter().kinds([Kind(5109), Kind(7000), Kind(6109)]).since(one_hour_ago)
