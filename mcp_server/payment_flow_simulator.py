...
class PaymentFlowSimulator(HandleNotification):
    ...
    async def handle(self, relay_url: str, subscription_id: str, ev: Event):
        ...
        if event_kind == 5109 and event_id_hex not in self.processing_events:  # Request event
    ...
    async def _process_request_event(self, relay_url: str, event: Event):
        ...
        # Step 2: Respond with a kind 7000 event (payment required)
        ...
        tags = [
            ...
            Tag.parse(["amount", "50"]),  # 50 sats as specified
        ]
        
        # Build the payment required event
        payment_builder = EventBuilder(Kind(7000), "Payment required to process your request").tags(tags)
        ...
        print(f"PAYMENT REQUIRED sent: {payment_event_id[:8]}... - Amount: 50 sats")
        
        # Step 3: Wait a few seconds (simulating payment processing)
        print(f"Waiting for payment simulation (3 seconds)...")
        await asyncio.sleep(3)
        
        # Step 4: Send a kind 6109 job result
        print(f"Payment simulation complete. Sending job result for request {event_id[:8]}...")
        ...
        result_builder = EventBuilder(Kind(6109), json.dumps(result_data)).tags(result_tags)
